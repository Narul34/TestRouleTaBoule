	...
	public void dessine(){
		...
		for (int i = 0; i < 30; i++) {
			for (int j = 0; j < 30; j++) {
				...
				this.getGraphics().drawImage(GenerateurCoordCarte.element[i][j].getImage(), GenerateurCoordCarte.element[i][j].getCoordonnees().getX(),
						GenerateurCoordCarte.element[i][j].getCoordonnees().getY(), this);
				this.getGraphics().drawImage(boule.getImage(), boule.getCoord().getX(), boule.getCoord().getY(), this);
			}
		}
	}
}
