...
public class JCarte extends JPanel {
	...
	public JCarte() {
		...
		panMap.setBounds(0, fenetre.mnuMenuBar.getHeight(), imageMap.getIcon().getIconWidth(),
				imageMap.getIcon().getIconHeight());
		panMap.add(imageMap);
	...
	public void dessine() {
		...
		for (int i = 0; i < 30; i++) {
			for (int j = 0; j < 30; j++) {
				bI.getGraphics().drawImage(GenerateurCoordCarte.element[i][j].getImage(),
						GenerateurCoordCarte.element[i][j].getCoordonnees().getX(),
